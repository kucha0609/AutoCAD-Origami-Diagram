...
;OrigamiTools的定位
	...
	OrigamiTools是AutoCAD的一个插件，无法独立运行。所以你必须在安装任意一个版本的AutoCAD之后，再安装OrigamiTools
		...
		这里推荐AutoCAD的版本大于等于2018，低版本可能会缺少某些函数
		你可以到AutoCAD官网申请一个月的软件试用，以便您可以更好的了解它
		请不要安装精简版的AutoCAD，可能会缺少某些函数
...
;参数说明 and 调用的命令:
	...
	功能01：页码递增：仅适用于含属性文字的块
  	...
  	功能03：文字递增刷：连续刷文字可呈递增数列
		命令:DZ   
  	功能04：文字对齐：快速调整文字之间的相对位置
		命令:QAT  
  	功能05：超级修剪：修剪闭合区域内部或外部的对象
		命令:EXTRIM   
  	功能06：分解属性块：带属性文字的块不能直接使用X命令进行分解
		命令:BURST
