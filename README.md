...
※[《Description for OT》](https://github.com/kucha0609/AutoCAD-Origami-Diagram/blob/master/--Info--/%E3%80%8ADescription%20for%20OT%E3%80%8B.pdf)
	...
	功能13：重做填充：使用拾取对象的方式重新生成填充
		命令:RH/Redo_Hatch
	功能14：一键转换：填充分离FU+重做填充RH+颜色转换CC+字段转换TOTEXT+对象顺序LO
		命令:TOAI/`TOAI 
	...
	功能16：文字替换：用字符串A中的"内容"替换掉字符串B中的"内容"
		命令:FA/Fill_All 
	功能17：伸缩折线：将所选择的直线伸长或缩短(正值则伸长,负值则缩短)，默认值是"-1"
		命令:DE/Delta
	功能18：缩放线型比例：根据给定的数值缩放所选对象的线型比例(支持嵌套块)(不支持天正对象)
		命令:LTS/Linetype_Scale  
	功能19：填充分离：创建独立的填充和解除图案关联
		命令:FU/Fill_Unlink
	功能20：数值增加：所选字符串前的数字统一增加设定的值(负值则为减少)
		命令:NI/Num_increases 		
		
	功能21：图层管理：采用数字+汉语拼音简写的方式来调用对应的图层状态
...
---------------------------------------------------------------------------------------------------------------
	...
	功能01：页码递增：仅适用于含属性文字的块
  	...
  	功能02：文字递增刷：连续刷文字可呈递增数列
		命令:DZ   ←"递增"的拼音   
  	功能03：超级修剪：修剪内部或外部的对象
		命令:CTR ← Circle TRIM
  	功能04：分解属性块：因为带属性文字的块不能直接使用X命令进行分解
		命令:BURST
