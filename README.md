...
※[《Description for OT》](https://github.com/kucha0609/AutoCAD-Origami-Diagram/blob/master/--Info--/%E3%80%8ADescription%20for%20OT%E3%80%8B.pdf)
	...
	功能16：伸缩折线：将所选择的直线伸长或缩短(正值则伸长,负值则缩短)，默认值是"-1"
		命令:DE/Delta
	功能17：缩放线型比例：根据给定的数值缩放所选对象的线型比例(支持嵌套块)(不支持天正对象)
		命令:LTS/Linetype_Scale  
	功能18：填充分离：创建独立的填充和解除图案关联
		命令:FU/Fill_Unlink
	功能19：数值增加：所选字符串前的数字统一增加设定的值(负值则为减少)
		命令:NI/Num_increases 		
		
	功能20：图层管理：采用数字+汉语拼音简写的方式来调用对应的图层状态
