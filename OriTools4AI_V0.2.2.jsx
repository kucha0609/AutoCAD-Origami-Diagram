...
//Global:定义窗口和按钮功能------------------------------------------
    ...
    var RELayS = SubfPnl.add('checkbox', undefined, '恢复图层状态');//Remove_ExistArtbd
        RELayS.value = true;
    */
    var MNote = MainPnl.add('statictext', undefined, '一键快捷含子功能01～08');
        MNote.justify = 'Left';
        MNote.enabled = false;


    var BTS01 = SubfPnl.add('button', undefined, '01-层按名排序');//ChgLayOrder
    var BTS02 = SubfPnl.add('button', undefined, '02-解除所有组');//Ungroup_All
    var MskNote = SubfPnl.add('checkbox', undefined, '解组时保留剪切蒙版');//Remove_Clipping_Masks
        MskNote.value = true;
    var BTS03 = SubfPnl.add('button', undefined, '03-删字段背景');//Remove_FieldBG
    var BTS04 = SubfPnl.add('button', undefined, '04-移动绿色线');//Mv_GreenLine2Lay
    var BTS05 = SubfPnl.add('button', undefined, '05-改色及线型');//Chg_ColFormat

    var BTS11 = SubfPnl.add('button', undefined, '06-矩形转画板');//Obj_2Artboard
    var ArtNote01 = SubfPnl.add('checkbox', undefined, '删除旧的画板');//Remove_ExistArtbd
        ArtNote01.value = true;
    var ArtNote02 = SubfPnl.add('checkbox', undefined, '关闭图框层和零层');//UnShow_PageLay
        ArtNote02.value = true; 

    var BTS12 = SubfPnl.add('button', undefined, '07-文字置顶层');//Text_2Front
    var BTS13 = SubfPnl.add('button', undefined, '08-白色置底层');//White_2Back

    var BTF01 = FilePnl.add('button', undefined, '另存为.AI');//saveAs_AI
    var BTF02 = FilePnl.add('button', undefined, '另存为PDF');//saveAs_PDF
    var BTF03 = FilePnl.add('button', undefined, '另存为JPE');//saveAs_JPEG

    //按钮尺寸统一，也可分别设置
    BTM01.size = BTM02.size = [200, 20];
    BTS01.size = BTS02.size = BTS03.size = BTS04.size = BTS05.size = [200, 20]
    ArtNote01.size = ArtNote02.size = [200, 20]
    BTS11.size = BTS12.size = BTS13.size = [200, 20]
    MskNote.size = [200, 20]
    BTF01.size = BTF02.size = BTF03.size = [200, 20];

    // 版权信息
    var Copyright = OriWin.add('statictext', undefined, '\u00A9 Kucha 2023');
    ...
    //
    BTS01.onClick = function () { Chg_LayOrder(); alert('\n图层顺序已按名称调整') }
    BTS02.onClick = function () { OriWin.close(); Ungroup_All(); alert('\n所有成组对象都已解组') }
    BTS03.onClick = function () { if (Remove_FieldBG()) { alert('\n已删除字段背景RGB192灰面'); } else { alert('\n文档中未找到RGB192的灰面'); } }
    BTS04.onClick = function () { if (Mv_GreenLine2Lay()) { alert('\n绿线已移到图层<D-05-Auxiliary>'); } else { alert('\n文档中未找到G255的绿线'); } }
    BTS05.onClick = function () { Chg_ColFormat(); alert('\n层内对象的颜色已调整'); }

    BTS11.onClick = function() {Obj_2Artboard()}
    BTS12.onClick = function() {Text_2Front()}
    BTS13.onClick = function() {White_2Back()}
    //
    BTF01.onClick = function() {saveAs_Ai()}
    BTF02.onClick = function() {SaveAs_PDF()}
    BTF03.onClick = function() {SaveAs_JPG()}
    //-----------------------------------------------------
...
//~function_01------------------------------------------------------
    ...
    function Chg_LayOrder(){//~~Adjust layer order (Alphabetized)  
    ...
    function Obj_2Artboard() {//~~Create Artboard from multiple objects
        ...
        function CompareRecBounds(Rec01, Rec02) {//比较矩形对象的几何边界：先上下后左右
            ...
            //按先上下后左右的顺序重新排列对象的顺序
            if (Rec01Bounds[1] === Rec02Bounds[1]) {//左上角Y的值和类型相等
        ...
        if (TgtLay && RecLst4Lay[0]) {
            ...
            if(ArtNote02.value){
                ...
                if (TgtLay2){TgtLay2.visible = false;}
            }//关闭图层
        } else {
            alert('\n文档不存在图层<D-16-Page>\n或图层<D-16-Page>不含矩形');
        };
...
//~function_02------------------------------------------------------
    function saveAs_Ai(){//~~Save the file in AI format
        ...
        CurDoc.saveAs(aiDoc, saveOptions);
        app.redraw();
        }
    function SaveAs_PDF(){//~~Save the file in PDF format
        //Get the path where the script is located
            //var scriptFile = new File($.fileName);
        ...
        // saveOptions.compatibility = Compatibility.ILLUSTRATOR8;
        // saveOptions.flattenOutput = OutputFlattening.PRESERVEAPPEARANCE;
        CurDoc.saveAs(pdfName, saveOptions);
        app.redraw();
        }
    function SaveAs_JPG() {//~~Save the file in PDF format
        //Get the path where the script is located
            //var scriptFile = new File($.fileName);
        ...
        for(var i = 0; i < DocArtbds.length; i++) {
            DocArtbds.setActiveArtboardIndex(i);
        ...
        for (var i = DocLayers.length - 1; i >= 0; i--) {
            ...
            for (var j = 0; j < GlbLayStatus_Visible.length; j++) {
                if (CurLayNam === GlbLayStatus_Visible[j]) {
                    DocLayers[i].visible = false;//关闭图层
                }
            }
            for (var j = 0; j < GlbLayStatus_UnLcked.length; j++) {
                if (CurLayNam === GlbLayStatus_UnLcked[j]) {
                    DocLayers[i].locked = true;//锁定图层
                }
            }
        }
    }//根据图层名称恢复图层状态
